...
export function PlanParticipation({
  ...
  async function handleJoin() {
    ...
    setError(null);
    setMessage(null);
    const response = await fetch(`/api/plans/${planId}/join`, { method: "POST" });
    const payload = await response.json().catch(() => null);
    setLoading(false);
  ...
  async function handleLeave() {
    ...
    setError(null);
    setMessage(null);
    const response = await fetch(`/api/plans/${planId}/leave`, { method: "POST" });
    const payload = await response.json().catch(() => null);
    setLoading(false);
  ...
  async function handleApprove(requestId: string) {
    const response = await fetch(
      `/api/plans/${planId}/participants/${requestId}/approve`,
      { method: "POST" }
    );
    const payload = await response.json().catch(() => null);
    if (!response.ok) {
  ...
  async function handleDecline(requestId: string) {
    const response = await fetch(
      `/api/plans/${planId}/participants/${requestId}/decline`,
      { method: "POST" }
    );
    const payload = await response.json().catch(() => null);
    if (!response.ok) {

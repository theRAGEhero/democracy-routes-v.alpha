...
export async function POST(
  ...
  const block = await prisma.planBlock.findFirst({
    where: { id: blockId, planId: plan.id, type: "RECORD" },
    select: { id: true }
  });
  if (!block) {
    return NextResponse.json({ error: "Record block not found" }, { status: 404 });
  }

  const audioBuffer = Buffer.from(await audio.arrayBuffer());
  const uploadsDir = path.join(process.cwd(), "data", "record-audio", "uploads");
  await fs.mkdir(uploadsDir, { recursive: true });
  const safeName = `${plan.id}-${viewer.user.id}-${Date.now()}-${audio.name || `record-${blockId}.webm`}`;
  ...
  for (const candidate of candidates) {
    try {
      createRoundResponse = await fetch(`${candidate}/api/rounds`, {
        ...
        body: JSON.stringify({
          name: `${plan.title} - Record`,
          description: `Record block ${blockId}`,
  ...
  audioForm.append("filename", audio.name || `record-${blockId}.webm`);
  ...
  const sessionRecord = await prisma.planRecordSession.upsert({
    where: {
      blockId_userId: {
        blockId: block.id,
        userId: viewer.user.id

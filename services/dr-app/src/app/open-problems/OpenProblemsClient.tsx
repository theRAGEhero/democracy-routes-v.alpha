...
export function OpenProblemsClient({ initialProblems, dataspaces }: Props) {
  ...
  async function handlePublish() {
    ...
    try {
      ...
      setProblems((current) => [payload as ProblemEntry, ...current]);
      resetComposer();
      setDataspaceId("");
      setSaveStatus("Open problem published.");
    } catch (nextError) {
  ...
  async function handleSaveEdit(problemId: string) {
    ...
    try {
      ...
      setProblems((current) =>
        current.map((problem) => (problem.id === problemId ? (payload as ProblemEntry) : problem))
      );
      setSimilarProblems((current) =>
        current.map((problem) =>
          problem.id === problemId
      ...
      <section className="rounded-[28px] border border-[color:var(--stroke)] bg-[color:var(--card)] p-4 shadow-[0_18px_45px_rgba(15,23,42,0.08)] sm:p-5">
        ...
        {similarProblems.length > 0 ? (
          <div className="mt-4 rounded-3xl border border-amber-200 bg-amber-50/80 p-4">
            <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-amber-700">Similar open problems</div>
        ...
        <div className="mt-5 flex justify-end">
          ...
          >
            {saving ? "Publishing..." : "Publish open problem"}
          </button>
        </div>
      </section>
        ...
        <div className="mt-4 space-y-3">
          {problems.length === 0 ? (
            ...
            problems.map((problem) => (
              <article key={problem.id} className="rounded-3xl border border-slate-200 bg-white px-4 py-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
